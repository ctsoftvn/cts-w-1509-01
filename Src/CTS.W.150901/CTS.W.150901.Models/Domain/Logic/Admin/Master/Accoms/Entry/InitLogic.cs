    ...
    public class InitLogic
    {
        ...
        private void Check(InitDataModel inputObject) {
            ...
            if (inputObject.IsEdit) {
                if (DataCheckHelper.IsNull(inputObject.AccomCd)) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00013", "ADM_MA_ACCOMS_00001"));
                }
                ...
                var isExist = masterDataCom.IsExistAccom(
                    basicLocale, inputObject.AccomCd, true);
                if (!isExist) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00016", "ADM_MA_ACCOMS_00005"));
                }
                // Kiểm tra danh sách lỗi
                if (!DataCheckHelper.IsNull(msgs)) {
                    throw new ExecuteException(msgs);
        ...
        private InitDataModel GetInfo(InitDataModel inputObject) {
            ...
            var localeModel = new LocaleModel<AccomObject>();
            // Map dữ liệu
            ...
            if (inputObject.IsEdit || inputObject.IsCopy) {
                // Khởi tạo biến cục bộ
                var listData = DataHelper.CreateList<AccomObject>();
                // Lấy danh sách dữ liệu đa ngôn ngữ
                var listLocale = processDao.GetListLocale(inputObject.AccomCd);
            ...
            if (inputObject.IsAdd) {
                localeModel.DataInfo.AccomCd = DataHelper.GetUniqueKey();
                localeModel.DataInfo.AccomName = string.Empty;

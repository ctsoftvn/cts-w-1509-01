...
using CTS.W._150901.Models.Domain.Model.Admin.Master.Accoms.Entry;
using CTS.W._150901.Models.Domain.Object.Admin.Master;

namespace CTS.W._150901.Models.Domain.Logic.Admin.Master.Accoms.Entry
{
    /// <summary>
    /// SaveLogic
    /// </summary>
    public class SaveLogic
    {
        ...
        private void Check(SaveDataModel inputObject) {
            ...
            if (DataCheckHelper.IsNull(dataInfo.AccomCd)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_ACCOMS_00001"));
            }
            if (DataCheckHelper.IsNull(dataInfo.AccomName)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00001", "ADM_MA_ACCOMS_00002"));
            }
            ...
            var isExist = masterDataCom.IsExistAccom(
                dataInfo.LocaleCd, dataInfo.AccomCd, true);
            // Kiểm tra dữ liệu tồn tại trường hợp status là add
            if (inputObject.IsAdd && (isExist)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00017", "ADM_MA_ACCOMS_00005"));
            }
            // Kiểm tra dữ liệu tồn tại trường hợp status là edit
            if (inputObject.IsEdit && (!isExist)) {
                msgs.Add(MessageHelper.GetMessage("E_MSG_00016", "ADM_MA_ACCOMS_00005"));
            }
            // Kiểm tra danh sách lỗi
            if (!DataCheckHelper.IsNull(msgs)) {
                throw new ExecuteException(msgs);
            ...
            // var isUnique = masterDataCom.IsUniqueAccom(dataInfo.AccomCd, dataInfo.Slug);
            // if (!isUnique) {
            ...
            foreach (var info in listLocale) {
                ...
                if (DataCheckHelper.IsNull(info.AccomName)) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "P_CM_00020", i, "E_MSG_00001", "ADM_MA_ACCOMS_00002"));
                }

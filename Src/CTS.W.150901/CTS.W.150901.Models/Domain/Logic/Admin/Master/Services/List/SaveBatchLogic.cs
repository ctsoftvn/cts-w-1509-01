    ...
    public class SaveBatchLogic
    {
        ...
        private void Check(SaveBatchDataModel inputObject) {
            ...
            foreach (var info in inputObject.ListData) {
                ...
                if (DataCheckHelper.IsNull(info.ServiceCd)) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "P_CM_00015", i, "E_MSG_00001", "ADM_MA_SERVICES_00001"));
                }
                if (DataCheckHelper.IsNull(info.ServiceName)) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "P_CM_00015", i, "E_MSG_00001", "ADM_MA_SERVICES_00002"));
                }
                ...
                var isExist = masterDataCom.IsExistService(
                    info.LocaleCd, info.ServiceCd, true);
                if (!isExist) {
                    flagError = true;
                    msgs.Add(MessageHelper.GetMessageForList(
                        "P_CM_00015", i, "E_MSG_00016", "ADM_MA_SERVICES_00005"));
                }
                // Trường hợp lỗi thì đi đến record tiếp theo
                if (flagError) {
                    // Tăng giá trị i
                ...
                // var isUnique = masterDataCom.IsUniqueService(info.ServiceCd, info.Slug);

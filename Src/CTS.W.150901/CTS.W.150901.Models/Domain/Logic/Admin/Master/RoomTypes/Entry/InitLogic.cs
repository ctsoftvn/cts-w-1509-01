...
namespace CTS.W._150901.Models.Domain.Logic.Admin.Master.RoomTypes.Entry
{
    ...
    public class InitLogic
    {
            ...
            if (inputObject.IsEdit) {
                if (DataCheckHelper.IsNull(inputObject.TypeCd)) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00013", "ADM_MA_ROOMTYPES_00001"));
                }
                ...
                var isExist = masterDataCom.IsExistRoomType(
                    basicLocale, inputObject.TypeCd, true);
                if (!isExist) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00016", "ADM_MA_ROOMTYPES_00006"));
                }
                // Kiểm tra danh sách lỗi
                if (!DataCheckHelper.IsNull(msgs)) {
                    throw new ExecuteException(msgs);
        ...
        private InitDataModel GetInfo(InitDataModel inputObject) {
            ...
            var localeModel = new LocaleModel<RoomTypeObject>();
            // Map dữ liệu
            ...
            if (inputObject.IsEdit || inputObject.IsCopy) {
                // Khởi tạo biến cục bộ
                var listData = DataHelper.CreateList<RoomTypeObject>();
                // Lấy danh sách dữ liệu đa ngôn ngữ
                var listLocale = processDao.GetListLocale(inputObject.TypeCd);
            ...
            if (inputObject.IsAdd) {
                localeModel.DataInfo.TypeCd = DataHelper.GetUniqueKey();
                localeModel.DataInfo.TypeName = string.Empty;

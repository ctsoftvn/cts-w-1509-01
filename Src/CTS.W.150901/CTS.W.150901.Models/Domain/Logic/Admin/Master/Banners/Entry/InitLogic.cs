...
using CTS.W._150901.Models.Domain.Model.Admin.Master.Banners.Entry;
using CTS.W._150901.Models.Domain.Object.Admin.Master;
using CTS.Web.Core.Domain.Helper;

namespace CTS.W._150901.Models.Domain.Logic.Admin.Master.Banners.Entry
{
    /// <summary>
    /// InitLogic
    /// </summary>
    public class InitLogic
    {
            ...
            if (inputObject.IsEdit) {
                if (DataCheckHelper.IsNull(inputObject.BannerCd)) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00013", "ADM_MA_BANNERS_00001"));
                }
                ...
                var isExist = masterDataCom.IsExistBanner(
                    basicLocale, inputObject.BannerCd, true);
                if (!isExist) {
                    msgs.Add(MessageHelper.GetMessage(
                        "E_MSG_00016", "ADM_MA_BANNERS_00005"));
                }
                // Kiểm tra danh sách lỗi
                if (!DataCheckHelper.IsNull(msgs)) {
                    throw new ExecuteException(msgs);
        ...
        private InitDataModel GetInfo(InitDataModel inputObject) {
            ...
            var localeModel = new LocaleModel<BannerObject>();
            // Map dữ liệu
            ...
            if (inputObject.IsEdit || inputObject.IsCopy) {
                // Khởi tạo biến cục bộ
                var listData = DataHelper.CreateList<BannerObject>();
                // Lấy danh sách dữ liệu đa ngôn ngữ
                var listLocale = processDao.GetListLocale(inputObject.BannerCd);
            ...
            if (inputObject.IsAdd) {
                localeModel.DataInfo.BannerCd = DataHelper.GetUniqueKey();
                localeModel.DataInfo.BannerName = string.Empty;

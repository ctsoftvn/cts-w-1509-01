// MABannersEntryCtrl
ctrls.controller('MABannersEntryCtrl', ['$scope', '$state', '$stateParams', '$window', function ($scope, $state, $stateParams, $window) {
    ...
    $scope.genCd = function () {
        $pc({
            ...
            success: function (data) {
                if ($scope.variable.BasicInfo.IsAdd && data.BannerCd) {
                    $scope.objEntry.DataInfo.BannerCd = data.BannerCd;
                }
            }
        });
    };
    ...
    $scope.uploadImage = function (obj) {
        $pc(function () {
            var modalInstance = $dialogHelper.showDialogUpload({
                ...
                FileCd: obj.FileCd,
                HasGen: obj.HasGen,
                FileGroupCd: 'stg.ma.banners.file-cd'
            });
            // Lấy kết quả xử lý
    ...
    $scope.fixRow = function () {
        $pc(function () {
            ...
            if ($scope.data.BasicLocale === obj.LocaleCd) {
                $dialogHelper.showDialogError($resourceHelper.getMessage('E.MSG.00004', 'ADM.MA.BANNERS.ENTRY.LocaleCd'));
                return;
            }
            // Lấy số dòng hợp lệ
    ...
    $scope.copyInfo = function () {
        var fnYes = function () {
            $pc(function () {
                $scope.objLocale.BannerName = $scope.objEntry.DataInfo.BannerName;
    ...
    // Xử lý thay đổi tên
    $scope.blurBannerName = function (data, obj) {
        $pc(function () {
    ...
    $scope.genSearchName = function (obj) {
        obj.SearchName = $dataHelper.toSearchName(obj.BannerName);
    };

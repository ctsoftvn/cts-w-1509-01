// MARoomTypesEntryCtrl
ctrls.controller('MARoomTypesEntryCtrl', ['$scope', '$state', '$stateParams', '$window', function ($scope, $state, $stateParams, $window) {
    ...
    $scope.variable.ShowLinkBack = $state.is('master-room-types-list-entry');
    ...
    $scope.init = function () {
        $pc({
            action: 'InitLayout',
            url: '/adm/hdl/ma/room-types/entry.ashx',
            data: {
    ...
    $scope.back = function () {
        $pc(function () {
            $state.go('master-room-types-list');
        });
    };
    // Tạo mới mã
    ...
    $scope.uploadImage = function (obj) {
        $pc(function () {
            var modalInstance = $dialogHelper.showDialogUpload({
                ...
                FileCd: obj.FileCd,
                HasGen: obj.HasGen,
                FileGroupCd: 'stg.ma.room-types.file-cd'
            });
            // Lấy kết quả xử lý
    ...
    $scope.fixRow = function () {
        $pc(function () {
            ...
            if ($scope.data.BasicLocale === obj.LocaleCd) {
                $dialogHelper.showDialogError($resourceHelper.getMessage('E.MSG.00004', 'ADM.MA.ROOMTYPES.ENTRY.LocaleCd'));
                return;
            }
            // Lấy số dòng hợp lệ

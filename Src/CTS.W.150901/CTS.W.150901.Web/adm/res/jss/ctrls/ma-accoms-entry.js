// MAAccomsEntryCtrl
ctrls.controller('MAAccomsEntryCtrl', ['$scope', '$state', '$stateParams', '$window', function ($scope, $state, $stateParams, $window) {
    ...
    $scope.variable.ShowLinkBack = $state.is('master-accoms-list-entry');
    ...
    $scope.genCd = function () {
        $pc({
            ...
            success: function (data) {
                if ($scope.variable.BasicInfo.IsAdd && data.AccomCd) {
                    $scope.objEntry.DataInfo.AccomCd = data.AccomCd;
                }
            }
        });
    };
    ...
    $scope.fixRow = function () {
        $pc(function () {
            ...
            if ($scope.data.BasicLocale === obj.LocaleCd) {
                $dialogHelper.showDialogError($resourceHelper.getMessage('E.MSG.00004', 'ADM.MA.ACCOMS.ENTRY.LocaleCd'));
                return;
            }
            // Lấy số dòng hợp lệ
    ...
    $scope.copyInfo = function () {
        var fnYes = function () {
            $pc(function () {
                $scope.objLocale.AccomName = $scope.objEntry.DataInfo.AccomName;
    ...
    // Xử lý thay đổi tên
    $scope.blurAccomName = function (data, obj, genSlug) {
        $pc(function () {
            // if (genSlug) {
            //     $scope.genSlug(obj);
            // }
    ...
    //         obj.Slug = $dataHelper.toSlug(obj.AccomName);
    //     }
    // };

...
ctrls.controller('MARoomTypesListCtrl', ['$scope', '$state', '$stateParams', '$window', function ($scope, $state, $stateParams, $window) {
    ...
    $scope.tblResult.Options = $optionHelper.gridBase({ width: 930 });
    /* Định nghĩa phương thức xử lý */
    // Xử lý init
    $scope.init = function () {
        $pc({
            action: 'InitLayout',
            url: '/adm/hdl/ma/room-types/list.ashx',
    ...
    $scope.add = function () {
        var fnYes = function () {
            $pc(function () {
                ...
                $state.go('master-room-types-list-entry', params);
            });
        };

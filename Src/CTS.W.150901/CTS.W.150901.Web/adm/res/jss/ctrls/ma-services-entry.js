// MAServicesEntryCtrl
ctrls.controller('MAServicesEntryCtrl', ['$scope', '$state', '$stateParams', '$window', function ($scope, $state, $stateParams, $window) {
    ...
    $scope.variable.ShowLinkBack = $state.is('master-services-list-entry');
    ...
    $scope.genCd = function () {
        $pc({
            ...
            success: function (data) {
                if ($scope.variable.BasicInfo.IsAdd && data.ServiceCd) {
                    $scope.objEntry.DataInfo.ServiceCd = data.ServiceCd;
                }
            }
        });
    };
    ...
    $scope.uploadImage = function (obj) {
        $pc(function () {
            var modalInstance = $dialogHelper.showDialogUpload({
                ...
                FileCd: obj.FileCd,
                HasGen: obj.HasGen,
                FileGroupCd: 'stg.ma.services.file-cd'
            });
            // Lấy kết quả xử lý
    ...
    $scope.fixRow = function () {
        $pc(function () {
            ...
            if ($scope.data.BasicLocale === obj.LocaleCd) {
                $dialogHelper.showDialogError($resourceHelper.getMessage('E.MSG.00004', 'ADM.MA.SERVICES.ENTRY.LocaleCd'));
                return;
            }
            // Lấy số dòng hợp lệ
    ...
    $scope.copyInfo = function () {
        var fnYes = function () {
            $pc(function () {
                $scope.objLocale.ServiceName = $scope.objEntry.DataInfo.ServiceName;
    ...
    // Xử lý thay đổi tên
    $scope.blurServiceName = function (data, obj, genSlug) {
        $pc(function () {
            // if (genSlug) {
            //     $scope.genSlug(obj);
            // }
    ...
    //         obj.Slug = $dataHelper.toSlug(obj.ServiceName);
    //     }
    // };
    // Tạo tự động tên tìm kiếm
    $scope.genSearchName = function (obj) {
        obj.SearchName = $dataHelper.toSearchName(obj.ServiceName);
    };

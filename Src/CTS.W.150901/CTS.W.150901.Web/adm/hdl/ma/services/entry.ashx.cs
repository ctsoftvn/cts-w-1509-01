...
using CTS.W._150901.Models.Domain.Logic.Admin.Master.Services.Entry;
...
namespace CTS.W._150901.Web.adm.hdl.ma.services
{
    ...
    public class entry : HandlerBase
    {
        ...
        private BasicResponse InitLayout(HttpContext context, BasicRequest request) {
            ...
            // Gán giá trị config
            config.RoleCd = W150901Logics.CD_ROLE_CD_ADM_MA_SRV_EN_VIEW;
            // Tiến hành gọi logic
        ...
        private BasicResponse Save(HttpContext context, BasicRequest request) {
            ...
            var config = new BasicConfig();
            // Gán giá trị config
            config.RoleCd = W150901Logics.CD_ROLE_CD_ADM_MA_SRV_EN_UPDATE;
            // Tiến hành gọi logic
            var response = HandlerCom.Invoke(logic, request, config);
            // Kết quả xử lý
            return response;
        ...
        public BasicResponse GenCd(HttpContext context, BasicRequest request) {
            ...
            if (request.IsAdd) {
                Thread.Sleep(1000);
                response.Add("ServiceCd", DataHelper.GetUniqueKey());

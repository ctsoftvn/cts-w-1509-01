...
using CTS.W._150901.Models.Domain.Logic.Admin.Master.Pages;
using CTS.Web.Core.Domain.Controller;
using CTS.Web.Core.Domain.Model;

namespace CTS.W._150901.Web.adm.hdl.ma.pages
{
    /// <summary>
    ...
    public class list : HandlerBase
    {
        ...
        private BasicResponse InitLayout(HttpContext context, BasicRequest request) {
            ...
            // Gán giá trị config
            config.RoleCd = W150901Logics.CD_ROLE_CD_ADM_SE_PAG_VIEW;
            // Tiến hành gọi logic
        ...
        private BasicResponse SaveBatch(HttpContext context, BasicRequest request) {
            ...
            var config = new BasicConfig();
            // Gán giá trị config
            config.RoleCd = W150901Logics.CD_ROLE_CD_ADM_SE_PAG_UPDATE;
            // Tiến hành gọi logic
            var response = HandlerCom.Invoke(logic, request, config);
            // Kết quả xử lý
            return response;
